  ...
  this.onMessage = function (socket, args) {
    ...
    self.actor(socket, function (err, actor) {
      ...
      self.target(socket, args, function (err, target) {
        ...
        event.target = target;
        event.content = args;
        self.publisher().emit('message', Message(event), socket);
      });
    });
  };
...
Messages.prototype.dettach = function (io) {
  if (io.sockets.fns.indexOf(this.middleware) >= 0) {
...
/**
 * Sets or Gets the publisher.  If it doesn't exist it will initialize it
 *
 * @param {EventEmitter} o * Optional
 * @return {mixed}
 */

Messages.prototype.publisher = function (o) {
  if (arguments.length === 0) {
    if (!this._publisher) {
      this._publisher = new events.EventEmitter();
    }
